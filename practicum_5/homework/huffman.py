...
class HuffmanCoding:
    ...
    def encode(self, sequence: list[Any]) -> str:
        ...
        while len(queue) > 1:
            counter += 1
            first_weight, (trash, first_node) = heapq.heappop(queue)
            second_weight, (trash, second_node) = heapq.heappop(queue)
    ...
    def decode(self, encoded_sequence: str) -> list[Any]:
        ...
        for bit in encoded_sequence:
            current += bit
            if current in self.codes_inversed:
                result.append(self.codes_inversed[current])
                current = ''
        return result
...
class LossyCompression:
    ...
    def compress(self, time_series: np.ndarray) -> str:
            ...
            interval = np.arange(self.seq_min, self.seq_max + step, step)
            
            values = [(interval[i] + interval[i + 1]) / 2 for i in range(len(interval) - 1)]
