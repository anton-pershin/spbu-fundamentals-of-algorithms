...
class LossyCompression:
    ...
    def compress(self, time_series: NDArrayFloat) -> str:
        self.seq_max = float(np.max(time_series))
        self.seq_min = float(np.min(time_series))
        self.seq_size = len(time_series)

        step = (self.seq_max - self.seq_min) / self.seq_size

        intervals = np.arange(self.seq_min, self.seq_max + step, step)

        values = [
            (intervals[i] + intervals[i + 1]) / 2
            for i in range(len(intervals) - 1)
        ]

        index_to_value = {i: values[i] for i in range(len(values))}
        quantized = []

        for x in time_series:
            idx = int((x - self.seq_min) / step)
            idx = min(idx, len(values) - 1)
            quantized.append(index_to_value[idx])

        self.HC = HuffmanCoding()
        return self.HC.encode(quantized)
