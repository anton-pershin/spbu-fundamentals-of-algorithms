...
class LuSolverWithPermute(LinearSystemSolver):
    ...
    def solve(self, b: NDArrayFloat) -> NDArrayFloat:
        n = self.A.shape[0]
        b_p = self.P @ b
        y = np.zeros_like(b_p, dtype=self.dtype)

        for i in range(n):
            y[i] = (b_p[i] - (self.L[i, :i] @ y[:i]))
        
        x = np.zeros_like(b_p, dtype=self.dtype)
        for i in range(n-1, -1, -1):
            x[i] = (y[i] - (self.U[i, i+1:] @ x[i+1:])) / self.U[i, i]
        return x

    def _decompose(self, permute: bool) -> tuple[NDArrayFloat, NDArrayFloat]:
