...
class ShortestPathLinearProgram:
    ...
    def solve(self, s_node: Any, t_node: Any) -> set[tuple[Any, Any]]:
        ...
        A_eq = np.zeros((len(self.nodes), len(c)), dtype=np.int64)
        node_indecies = list(range(n_nodes))
        for i in range(n_nodes):
            rowcol_selector = copy(node_indecies)
        ...
        #Возвратить множества ребер, составляющий кратчайший путь
