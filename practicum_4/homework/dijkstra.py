from pathlib import Path
from typing import Any
from abc import ABC, abstractmethod

import numpy as np
import networkx as nx
import heapq

from practicum_4.dfs import GraphTraversal 
from src.plotting.graphs import plot_graph
from src.common import AnyNxGraph


class DijkstraAlgorithm(GraphTraversal):
    def __init__(self, G: AnyNxGraph) -> None:
        self.shortest_paths: dict[Any, list[Any]] = {}
        super().__init__(G)

    def previsit(self, node: Any, **params) -> None:
        """List of params:
        * path: list[Any] (path from the initial node to the given node)
        """
        self.shortest_paths[node] = params["path"]

    def postvisit(self, node: Any, **params) -> None:
        pass

    def run(self, node: Any) -> None:
        dist = {node: float('inf') for node in self.G.nodes()}
        paths = {node: [] for node in self.G.nodes()}
        
        dist[node] = 0
        paths[node] = [node]                     

        unvisited = set(self.G.nodes())            

        while unvisited:
            cur_node = min(unvisited, key=lambda n: dist[n])
            if dist[cur_node] == float('inf'):    
                break

            self.visited.add(cur_node)
            self.previsit(cur_node, path=paths[cur_node])
            unvisited.remove(cur_node)

            for n in self.G.neighbors(cur_node):
                weight = self.G[cur_node][n].get('weight', 1.0)
                new_dist = dist[cur_node] + weight
                if new_dist < dist[n]:
                    dist[n] = new_dist
                    paths[n] = paths[cur_node] + [n]


if __name__ == "__main__":
    G = nx.read_edgelist(
        Path("practicum_4") / "simple_weighted_graph_9_nodes.edgelist",
        create_using=nx.Graph
    )
    plot_graph(G)

    sp = DijkstraAlgorithm(G)
    sp.run("0")

    test_node = "5"
    shortest_path_edges = [
        (sp.shortest_paths[test_node][i], sp.shortest_paths[test_node][i + 1])
        for i in range(len(sp.shortest_paths[test_node]) - 1)
    ]
    plot_graph(G, highlighted_edges=shortest_path_edges)

